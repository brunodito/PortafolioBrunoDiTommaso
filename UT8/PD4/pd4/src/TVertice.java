...
public class TVertice<T> implements IVertice {
    ...
    public void bea(Collection<TVertice> visitados){
        for (Object adyacencia : this.getAdyacentes()) {
            TAdyacencia ady = (TAdyacencia)  adyacencia;
            if(!visitados.contains(ady.getDestino())){
    ...
    public void recorrer(){
        if (!this.getVisitado()) {
            this.setVisitado(true);
            for (TAdyacencia adyacencia : adyacentes) {
                adyacencia.getDestino().recorrer();
            }
        }
    }

    public boolean esFuertementeConexo2(Object[] vertices){
        this.recorrer();
        for (Object vert : vertices) {
            TVertice vertice = (TVertice) vert;
