...
public class UtilGrafos {
    ...
    public static Double[][] obtenerMatrizCostos(Map<Comparable, Ciudad> vertices) {
        ...
        while (i < cantidadVertices) {
            ...
            while (j < cantidadVertices) {
                Ciudad elemVerticeI = vertices.get(VerticesIArr[i]);
                Ciudad elemVerticeJ = vertices.get(VerticesJArr[j]);

                if (!elemVerticeI.getEtiqueta().equals(elemVerticeJ.getEtiqueta())) {
                    Ciudad verticeI = (Ciudad) elemVerticeI;
                    Ciudad verticeJ = (Ciudad) elemVerticeJ;
    ...
    public static void imprimirMatriz(Comparable[][] matriz, Map<Comparable, Ciudad> vertices) {
    ...
    public static void imprimirMatrizMejorado(Comparable[][] matriz, Map<Comparable, Ciudad> vertices, String titulo) {
    ...
    public static <T extends IGrafoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, 
        ...
        List<Ciudad> verticesList = new ArrayList<Ciudad>(vertices.length);
        List<Ruta> aristasList = new ArrayList<Ruta>(aristas.length);

        for (String verticeString : vertices) {
            if ((verticeString != null) && (verticeString.trim() != "")) {
                verticeString = verticeString.split(",")[0];
                verticesList.add(new Ciudad(verticeString));
            }
        }
        for (String aristaString : aristas) {
            if ((aristaString != null) && (aristaString.trim() != "")) {
                String[] datos = aristaString.split(",");
                aristasList.add(new Ruta(datos[0], datos[1], Double.parseDouble(datos[2]), Boolean.parseBoolean(datos[3])));
            }
        }
        try {
