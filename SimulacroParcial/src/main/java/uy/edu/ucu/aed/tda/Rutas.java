...
public class Rutas extends LinkedList<Ruta> {
     ...
     * @return
     */
    public Ruta buscar(Comparable etOrigen, Comparable etDestino) {
        Ruta tempArista;
        for (Ruta laa : this) {
     ...
     * @return
     */
    public Ruta buscarMin(Collection<Comparable> VerticesU, Collection<Comparable> VerticesV) {
        Ruta tempArista;
        Ruta tAMin = null;
    ...
    void insertarAmbosSentidos(Collection<Ruta> aristas) {
        for (Ruta ta : aristas) {
            this.add(ta);
