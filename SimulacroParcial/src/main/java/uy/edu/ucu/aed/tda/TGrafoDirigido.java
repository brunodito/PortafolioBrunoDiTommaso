...
public class TGrafoDirigido implements IGrafoDirigido {
    ...
    private final Map<Comparable, Ciudad> vertices; // lista de vertices del grafo.-

    public TGrafoDirigido(Collection<Ciudad> vertices, Collection<Ruta> aristas) {
    ...
    public boolean eliminarArista(Comparable nomVerticeOrigen, Comparable nomVerticeDestino) {
        if ((nomVerticeOrigen != null) && (nomVerticeDestino != null)) {
            Ciudad vertOrigen = buscarVertice(nomVerticeOrigen);
            if (vertOrigen != null) {
    ...
    public boolean existeArista(Comparable etiquetaOrigen, Comparable etiquetaDestino) {
        Ciudad vertOrigen = buscarVertice(etiquetaOrigen);
        Ciudad vertDestino = buscarVertice(etiquetaDestino);
     ...
     */
    public Ciudad buscarVertice(Comparable unaEtiqueta) {
    ...
    public boolean insertarArista(Ruta arista) {
        ...
        if ((arista.getEtiquetaOrigen() != null) && (arista.getEtiquetaDestino() != null)) {
            Ciudad vertOrigen = buscarVertice(arista.getEtiquetaOrigen());
            Ciudad vertDestino = buscarVertice(arista.getEtiquetaDestino());
    ...
    public boolean insertarVertice(Comparable unaEtiqueta) {
        if ((unaEtiqueta != null) && (!existeVertice(unaEtiqueta))) {
            Ciudad vert = new Ciudad(unaEtiqueta);
    ...
    @Override
    public boolean insertarVertice(Ciudad vertice) {
    ...
    public Object[] getEtiquetasOrdenado() {
        TreeMap<Comparable, Ciudad> mapOrdenado = new TreeMap<>(this.getVertices());
        return mapOrdenado.keySet().toArray();
    }

    @Override
    public void desvisitarVertices() {
        for (Ciudad vertice : this.vertices.values()) {
     ...
     */
    @Override
    public Map<Comparable, Ciudad> getVertices() {
        return vertices;
    }

    @Override
    public Collection<Ciudad> bpf(Ciudad vertice) {
       this.desvisitarVertices();
        Collection<Ciudad> visitados=new LinkedList<Ciudad>();
        
        if(this.existeVertice(vertice.getEtiqueta()))
        {
            Ciudad vert=this.buscarVertice(vertice.getEtiqueta());
            vert.bpf(visitados);
        }
        return visitados;
    }
    ...
    @Override
    public Collection<Ciudad> bpf() {
        Collection<Ciudad> listaBpf = new LinkedList<Ciudad>();
        this.desvisitarVertices();
        
        if (vertices.isEmpty()) {
            System.out.println("El grafo está vacio");
        } else {
            for (Ciudad vertV : vertices.values()) {
    ...
    @Override
    public Collection<Ciudad> bpf(Comparable etiquetaOrigen) {
        this.desvisitarVertices();
        Collection<Ciudad> visitados=new LinkedList<Ciudad>();
        
        if(this.existeVertice(etiquetaOrigen))
        {
            Ciudad vertice=this.buscarVertice(etiquetaOrigen);
    ...
    public Comparable centroDelGrafo() {
        Iterator<Ciudad> it = vertices.values().iterator();
    ...
    public TCaminos todosLosCaminos(Comparable etiquetaOrigen, Comparable etiquetaDestino) {
        ...
        Ciudad v = buscarVertice(etiquetaOrigen);
        Ciudad u = buscarVertice(etiquetaDestino);
        if ((v != null)&&(u != null)) {
    ...
    public Collection<Ciudad> bea() {
       if (this.getVertices().isEmpty()) {
            ...
            this.desvisitarVertices();
            for (Ciudad vertV : this.getVertices().values()) {
                if (!vertV.getVisitado()) {
